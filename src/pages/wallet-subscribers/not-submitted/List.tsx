...
const List = () => {
  const [notSubmittedSubscribers, setNotSubmittedSubscribers] = useState<any[]>([]);
  ...
  const getPendingOtcProfiles = async (page: number, perPage: number) => {
    ...
    setLoading(false);
    if (res?.status === 200) {
      setNotSubmittedSubscribers(res?.data?.data);
  ...
  setHeaderTitle("not-submitted-subscribers");

  return (
    <Card title="Not Submitted List" className="my-6 shadow">
      <Row className="-mt-2">
        <Col
          xs={{ span: 24 }}
          sm={{ span: 24 }}
      ...
      <div className="mb-6 mt-2">
        <Table
          columns={columns}
          dataSource={notSubmittedSubscribers}
          rowKey="unique_id"
          pagination={false}
          size="small"
          expandable={{

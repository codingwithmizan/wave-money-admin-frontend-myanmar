...
const List = () => {
  const [rejectedSubscribers, setRejectedSubscribers] = useState<any[]>([]);
  ...
  const getPendingOtcProfiles = async (page: number, perPage: number) => {
    ...
    setLoading(false);
    if (res?.status === 200) {
      setRejectedSubscribers(res?.data?.data);
  ...
  setHeaderTitle("rejected-subscribers");
  ...
  return (
    <Card title="Rejected List" className="my-6 shadow">
      ...
      <div className="mb-6 mt-2">
        <Table
          columns={columns}
          dataSource={rejectedSubscribers}
          rowKey="unique_id"
          pagination={false}
          size="small"
          expandable={{

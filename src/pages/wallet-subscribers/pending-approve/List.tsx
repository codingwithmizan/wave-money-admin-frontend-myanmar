...
const List = () => {
  const [pendingApproveSubscribers, setPendingApproveSubscribers] = useState<any[]>([]);
  ...
  const getPendingOtcProfiles = async (page: number, perPage: number) => {
    ...
    setLoading(false);
    if (res?.status === 200) {
      setPendingApproveSubscribers(res?.data?.data);
  ...
  setHeaderTitle("pending-to-approve-subscribers");
  ...
  return (
    <Card title="Pending to Approve List" className="my-6 shadow">
      ...
      <div className="mb-6 mt-2">
        <Table
          columns={columns}
          dataSource={pendingApproveSubscribers}
          rowKey="unique_id"
          pagination={false}
          size="small"
          expandable={{

...
import { SubscribersFilter } from "@/components/wallet-subscribers";
...
const List = () => {
  const [recommendApproveSubscribers, setRecommendApproveSubscribers] = useState<any[]>([]);
  ...
  const getPendingOtcProfiles = async (page: number, perPage: number) => {
    ...
    setLoading(false);
    if (res?.status === 200) {
      setRecommendApproveSubscribers(res?.data?.data);
  ...
  setHeaderTitle("recommend-to-approve-subscribers");
  ...
  return (
    <Card title="Recommend to Approve List" className="my-6 shadow">
          ...
          <div className="flex items-center gap-6 mb-2 ">
            <form>
              <InputSearch
                control={control}
                name="search"
                placeholder="Search by MSISDN, Unique ID, ID Number or Name"
      ...
      <div className="mb-6 mt-2">
        <Table
          columns={columns}
          dataSource={recommendApproveSubscribers}
          rowKey="unique_id"
          pagination={false}
          size="small"
          expandable={{

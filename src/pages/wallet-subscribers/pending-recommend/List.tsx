...
const List = () => {
  const [pendingRecommendSubscribers, setPendingRecommendSubscribers] = useState<any[]>([]);
  ...
  const getPendingOtcProfiles = async (page: number, perPage: number) => {
    ...
    setLoading(false);
    if (res?.status === 200) {
      setPendingRecommendSubscribers(res?.data?.data);
  ...
  setHeaderTitle("pending-to-recommend-subscribers");
  ...
  return (
    <Card title="Pending to Recommend List" className="my-6 shadow">
      ...
      <div className="mb-6 mt-2">
        <Table
          columns={columns}
          dataSource={pendingRecommendSubscribers}
          rowKey="unique_id"
          pagination={false}
          size="small"
          expandable={{

...
const List = () => {
  const [approvedSubscribers, setApprovedSubscribers] = useState<any[]>([]);
  ...
  const getPendingOtcProfiles = async (page: number, perPage: number) => {
    ...
    setLoading(false);
    if (res?.status === 200) {
      setApprovedSubscribers(res?.data?.data);
  ...
  setHeaderTitle("approved-subscribers");
  ...
  return (
    <Card title="Approved List" className="my-6 shadow">
      ...
      <div className="mb-6 mt-2">
        <Table
          columns={columns}
          dataSource={approvedSubscribers}
          rowKey="unique_id"
          pagination={false}
          size="small"
          expandable={{

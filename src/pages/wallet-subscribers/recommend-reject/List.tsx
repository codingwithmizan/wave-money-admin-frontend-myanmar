...
const List = () => {
  const [recommendRejectSubscribers, setRecommendRejectSubscribers] = useState<any[]>([]);
  ...
  const getPendingOtcProfiles = async (page: number, perPage: number) => {
    ...
    setLoading(false);
    if (res?.status === 200) {
      setRecommendRejectSubscribers(res?.data?.data);
  ...
  setHeaderTitle("recommend-to-reject-subscribers");
  ...
  return (
    <Card title="Recommend to Reject List" className="my-6 shadow">
      ...
      <div className="mb-6 mt-2">
        <Table
          columns={columns}
          dataSource={recommendRejectSubscribers}
          rowKey="unique_id"
          pagination={false}
          size="small"
          expandable={{

...
const List = () => {
  const [recommendRejectOtcProfiles, setRecommendRejectOtcProfiles] = useState<any[]>([]);
  ...
  const getRejectedOtcProfiles = async (page: number, perPage: number) => {
    ...
    const res = await getPaginatedData(
      "otc_profiles/profile_recommendation/rejected",
      Object.assign(
        {
          page: page,
          perPage: perPage,
    ...
    setLoading(false);
    if (res?.status === 200) {
      setRecommendRejectOtcProfiles(res?.data?.data);
  ...
  return (
    <Card title="Recommend to Reject List" className="my-6 shadow">
      <Row className="-mt-2">
        <Col
          xs={{ span: 24 }}
          sm={{ span: 24 }}
      ...
      <div className="mb-6 mt-2">
        <Table
          columns={columns}
          dataSource={recommendRejectOtcProfiles}
          rowKey="unique_id"
          pagination={false}
          size="small"
          expandable={{
                  ...
                  <td>
                    <Space size="middle">
                      <Tooltip placement="bottom" title="OTC Details">
                        <Link
                          to={`/otc/recommend-to-rejects/details/${encodeURI(
                            record.unique_id
                          )}`}
                        >
...
const columns: any = [
  {
    ...
    render: (text: any, record: any) => (
      <Link
        to={`/otc/recommend-to-rejects/details/${encodeURI(record.unique_id)}`}
        className="underline"
      >
        {text}
      </Link>

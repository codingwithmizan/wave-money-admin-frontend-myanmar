...
import { setHeaderTitle, getStyledStatus } from "@/lib/helpers";
...
const List = () => {
  const [rejectedOtcProfiles, setRejectedOtcProfiles] = useState<any[]>([]);
  ...
  useEffect(() => {
    getRejectedOtcProfiles(page, perPage);
  }, [otcFilteredValue]);

  const getRejectedOtcProfiles = async (page: number, perPage: number) => {
    setLoading(true);
    const keys = {
    ...
    setLoading(false);
    if (res?.status === 200) {
      setRejectedOtcProfiles(res?.data?.data);
  ...
  return (
    <Card title="Rejected List" className="my-6 shadow">
      <Row className="-mt-2">
        <Col
          xs={{ span: 24 }}
          sm={{ span: 24 }}
      ...
      <div className="mb-6 mt-2">
        <Table
          columns={columns}
          dataSource={rejectedOtcProfiles}
          rowKey="unique_id"
          pagination={false}
          size="small"
          expandable={{
            expandedRowRender: (record) => (
              <table className="nested-table">
                <tr>
                  <td className="table-expand-data">Submitted Agent’s MSISDN :</td>
                  <td className="table-expand-data">{record?.agent_msisdn ? record?.agent_msisdn : "N/A"}</td>
                </tr>
                <tr>
                  <td className="table-expand-data">Profile Status :</td>
                  <td className="table-expand-data">{getStyledStatus(record?.account_status)}</td>
                </tr>
                <tr>
                  ...
                  <td>
                    <Space size="middle">
                      <Tooltip placement="bottom" title="OTC Details">
                        <Link
                          to={`/otc/rejected/details/${encodeURI(
                            record.unique_id
                          )}`}
                        >
                      ...
                      {/* <Tooltip title="OTC Edit">
                        <Link
                          to={`/otc/rejected/edit/${record.unique_id}`}
                        >
...
const columns: any = [
  {
    ...
    render: (text: any, record: any) => (
      <Link
        to={`/otc/rejected/details/${encodeURI(record.unique_id)}`}
        className="underline"
      >
        {text}
      </Link>
  ...
  },
  {
    ...
    render: (text: string) => (
      <span>{text === "nrc" ? "NRC" : "Passport"}</span>
    ),
  },
  {
    title: "ID Number  ",

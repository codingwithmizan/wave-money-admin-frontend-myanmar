...
const List = () => {
  const [reviewOtcProfiles, setReviewOtcProfiles] = useState<any[]>([]);
  ...
  useEffect(() => {
    getPendingToReviewOtcProfiles(page, perPage);
  }, [otcFilteredValue]);

  const getPendingToReviewOtcProfiles = async (
    page: number,
    perPage: number
  ) => {
    setLoading(true);
    ...
    const res = await getPaginatedData(
      "otc_profiles/pending/update_requests",
      Object.assign(
        {
          page: page,
          perPage: perPage,
    ...
    setLoading(false);
    if (res?.status === 200) {
      setReviewOtcProfiles(res?.data?.data);
  ...
  return (
    <Card title="Pending To Review Edited List" className="my-6 shadow">
      <Row className="-mt-2">
        <Col
          xs={{ span: 24 }}
          sm={{ span: 24 }}
                  ...
                  <td>
                    <Space size="middle">
                      <Tooltip placement="bottom" title="OTC Details">
                        <Link
                          to={`/otc/pending-to-review-edited/details/${encodeURI(
                            record.unique_id
                          )}`}
                        >
                      ...
                      {/* <Tooltip title="OTC Edit">
                        <Link
                          to={`/otc/pending-to-review-edited/edit/${record.unique_id}`}
                        >

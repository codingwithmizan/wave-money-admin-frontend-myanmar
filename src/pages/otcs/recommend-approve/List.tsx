...
const List = () => {
  const [recommendApproveOtcProfiles, setrRcommendApproveOtcProfiles] = useState<any[]>([]);
  ...
  const getAcceptOtcProfiles = async (page: number, perPage: number) => {
    ...
    const res = await getPaginatedData(
      "otc_profiles/profile_recommendation/approved",
      Object.assign(
        {
          page: page,
          perPage: perPage,
    ...
    setLoading(false);
    if (res?.status === 200) {
      setrRcommendApproveOtcProfiles(res?.data?.data);
  ...
  return (
    <Card title="Recommend to Approve List" className="my-6 shadow">
      <Row className="-mt-2">
        <Col
          xs={{ span: 24 }}
          sm={{ span: 24 }}
      ...
      <div className="mb-6 mt-2">
        <Table
          columns={columns}
          dataSource={recommendApproveOtcProfiles}
          rowKey="unique_id"
          pagination={false}
          size="small"
          expandable={{
                  ...
                  <td>
                    <Space size="middle">
                      <Tooltip placement="bottom" title="OTC Details">
                        <Link
                          to={`/otc/recommend-to-approves/details/${encodeURI(
                            record.unique_id
                          )}`}
                        >
...
const columns: any = [
  {
    ...
    render: (text: any, record: any) => (
      <Link
        to={`/otc/recommend-to-approves/details/${encodeURI(record.unique_id)}`}
        className="underline"
      >
        {text}
      </Link>
  ...
  },
  {
    ...
    align: "center",
    sorter: (a: any, b: any) => a?.identity_documents?.[0]?.id_number,
